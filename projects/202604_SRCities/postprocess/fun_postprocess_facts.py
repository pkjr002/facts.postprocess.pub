...
def convert_sampleTOquantiles(infile, outfile):
    ...
    with xr.open_dataset(infile) as nc:
        # Identify variables present in the file that match valid_vars
        ...
        infile_string = ", ".join("/".join(Path(f).parts[-3:]) for f in infile)
        # nc_description = nc.attrs.get("description", "").  # description for samples || Descriptionfor rates; use below
        ...
        for var in available_vars:
            ...
            # quantile_data = np.round(np.nanquantile(nc[var], q, axis=0), 3)
...
class Case:
    ...
    def source_dir(self, expFolder: str, ssp: str) -> Path:
        return Path(expFolder) / self.source_subdir.format(ssp=ssp)

    def file_name(self, ssp: str, wfcomp: str) -> str:
        return f"{self.prefix.format(ssp=ssp)}{wfcomp}"

    def rates_file_name(self, ssp: str, wfcomp: str) -> str:
        if wfcomp.endswith(".nc"):
            wfcomp = wfcomp[:-3] + "_rates.nc"
        else:
            wfcomp = wfcomp + "_rates"
        return f"{self.prefix.format(ssp=ssp)}{wfcomp}"
    
CASES = {
    # SSP-dependent
    "coupling": Case(source_subdir="coupling.{ssp}/output", prefix="coupling.{ssp}."),

    # SSP-dependent
    "src": Case(source_subdir="src.{ssp}/output", prefix="src.{ssp}."),
}
...
def postprocess(workflow, ssps, expFolder, folder2create, operation, case_key="coupling_ssp534_over"):
    
    if case_key not in CASES:
        raise ValueError(f"Unknown case_key '{case_key}'. Available: {list(CASES.keys())}")

    case = CASES[case_key]

    # Create top-level output folder
    Workflow_path = create_directory_structure(os.getcwd(), folder2create)
    out_root = Path(Workflow_path[0])

    
    skip_conditions = {
                
       'wf_3e': ['HL.ssp585', 'L.ssp245', 'LN.ssp245', 'M.ssp245' ,'VL.ssp126'],
       'wf_3f': ['HL.ssp585', 'L.ssp245', 'LN.ssp245', 'M.ssp245' ,'VL.ssp126'],
       'wf_4':  ['HL.ssp585', 'L.ssp245', 'LN.ssp245', 'M.ssp245' ,'VL.ssp126']
    }


    for indv_workflow in workflow:
        wf_path = create_directory_structure(out_root, indv_workflow)
        wf_dir = Path(wf_path[0])

        for ssp in ssps:

            if ssp in skip_conditions.get(indv_workflow, []):
               continue

            ssp_path = create_directory_structure(wf_dir, ssp)
            out_ssp_dir = Path(ssp_path[0])

            source_dir = case.source_dir(expFolder, ssp)
            
            rel = Path(indv_workflow) / ssp

            # Iterate over workflow components + totals
            for wfcomp in component[indv_workflow] + total[indv_workflow]:

                if operation == "create_workflow_folder":
                    src = source_dir / case.file_name(ssp, wfcomp)
                    _require_exists(src)
                    shutil.copy2(src, out_ssp_dir)

                elif operation == "convert_workflow_samples_to_quantiles":
                    inFile = Path.cwd() / "1_workflow" / rel / case.file_name(ssp, wfcomp)
                    outFile = out_ssp_dir / case.file_name(ssp, wfcomp)
                    _require_exists(inFile)
                    convert_sampleTOquantiles(str(inFile), str(outFile))

                elif operation == "create_workflow_rates":
                    inFile = Path.cwd() / "1_workflow" / rel / case.file_name(ssp, wfcomp)
                    outFile = out_ssp_dir / case.rates_file_name(ssp, wfcomp)
                    _require_exists(inFile)
                    create_workflow_rates(str(inFile), str(outFile))

                elif operation == "convert_workflow_rates_to_quantiles":
                    inFile = Path.cwd() / "1_workflow_rates" / rel / case.rates_file_name(ssp, wfcomp)
                    outFile = out_ssp_dir / case.rates_file_name(ssp, wfcomp)
                    _require_exists(inFile)
                    convert_sampleTOquantiles(str(inFile), str(outFile))
...
def create_pbox(infiles, outfile, pyear_start=2020, pyear_end=2300, pyear_step=10):
    ...
    # For quantile = p50 Calculate mean (of medians)across all input data.
    pbox[median_idx,:,:]    = np.mean(component_data[:,median_idx,:,:], axis=0)
    # For quantile < p50 Calculate min value across all input data.
    pbox[:median_idx,:,:]   = np.amin(component_data[:,:median_idx,:,:], axis=0)
    # For quantile > p50 Calculate max value across all input data.
    ...
    ds.attrs.update({"Description": "Pbox",
                     ...
                     "Author"     : "Praveen Kumar <praveen.kumar@rutgers.edu>",})
    # print("Global attributes before saving:", ds.attrs)
...
def process_indv_Pbox(indv_Pbox, ssp, ssp_path,region):
    wf_num = indv_Pbox.split('_')[1]
    
    folder_name = ssp_path.split(os.sep)[-3]
    mapping = {
        "3_pbox":           "2_workflow_quantiles",
        "3_pbox_rates":     "2_workflow_rates_quantiles"
    }
    ...
    pyear_end = 2150 if wf_num in ['1f', '2f'] else 2100
    ...
    create_pbox_from_component(base_paths, patterns, f"{ssp_path}/icesheets-pb{wf_num}-icesheets-{ssp}_AIS_{region}sl.nc", pyear_end)
    ...
    if indv_Pbox in ['pb_2e', 'pb_2f']:
        if indv_Pbox == 'pb_2e':
            ...
            create_pbox_from_component(base_paths, patterns, f"{ssp_path}/icesheets-pb{wf_num}-icesheets-{ssp}_GIS_{region}sl.nc", pyear_end)

            base_paths = [ssp_path, f"{os.getcwd()}/{workflow_folder}/wf_1f/{ssp}"]
            patterns = ["emulandice.glaciers", "ipccar5.glaciers"]
            create_pbox_from_component(base_paths, patterns, f"{ssp_path}/glaciers-pb{wf_num}-glaciers-{ssp}_{region}sl.nc", pyear_end)
...
def process_pbox(pbox,outFolderName,ssps):
    ...
    skip_conditions = {
       'pb_2e': ['HL.ssp585', 'L.ssp245', 'LN.ssp245', 'M.ssp245' ,'VL.ssp126'],
       'pb_2f': ['HL.ssp585', 'L.ssp245', 'LN.ssp245', 'M.ssp245' ,'VL.ssp126']
    }
    ...
    for indv_pbox in pbox:
        ...
        for ssp in ssps:
            if indv_pbox in skip_conditions and ssp in skip_conditions[indv_pbox]:
               continue
            
            ssp_path = create_directory_structure(pb_path, ssp)[0]
            process_indv_Pbox(indv_pbox, ssp, ssp_path,region)
...
def MakeConfidenceFile(infile_e=None, infile_f=None, f_years=np.arange(2020,2151,10), outfile=None, is_rates=False):
	...
	# nc_out[varname].values = np.round(nc_out[varname].values, 1) 
...
def GenerateConfidenceFiles(pboxdir, outdir):
	...
	if is_rates:
		...
		#Medium-confidence: pb_1f cut at  2150
